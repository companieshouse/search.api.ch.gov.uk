...
public class DisqualifiedSearchUpsertRequest {
    ...
    public String buildRequest(OfficerDisqualification officer) throws UpsertException, IOException {
        officer.getItems().stream().filter(item -> ! ObjectUtils.isEmpty(item.getAddress()))
            .findFirst().orElseThrow(() -> new UpsertException("Missing or empty mandatory Address field"));
